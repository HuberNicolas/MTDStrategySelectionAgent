...
while True:        
    ...
    for metricNumber, metric in zip(systemMetrics, METRICSNAME):
        ...
        print('\n')
    print(malwareIndicators, malwareIndicatorsNegative)

    indicatorResult = ''
    for k in malwareIndicatorsRelative:
        malwareIndicatorsRelative[k] = malwareIndicators[k] / (malwareIndicators[k] + malwareIndicatorsNegative[k])
        indicatorResult += '{}: {:.2f} '.format(MALWARETYPES[k], malwareIndicatorsRelative[k])
 
    # predict
    predictedType = max(malwareIndicatorsRelative, key=malwareIndicatorsRelative.get)  # todo: insert confidence band (e.g., over 95%)
