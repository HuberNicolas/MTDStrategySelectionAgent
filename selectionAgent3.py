...
while True:
    ...
    # maybe use 2nd line insead of first
            ...
            if metric == rule['metric']:
                ...
                if (rule[2] == '<=') & (float(metricNumber) <= float(rule[3])):
                    ...
                    observer.warning('{}|{}| Value: {}, Metric: {} {:.2f}: Possible {}'.format(
                        timestamp, metric, metricNumber, rule[2], rule[3], rule[0]))
    ...
    for k in malwareIndicatorsRelative:
        malwareIndicatorsRelative[k] = malwareIndicators[k] / \
            (malwareIndicators[k] + malwareIndicatorsNegative[k])
        indicatorResult += '{}: {:.2f} '.format(
            MALWARETYPES[k], malwareIndicatorsRelative[k])
